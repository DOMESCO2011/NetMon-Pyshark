...
def operators_window(master):
    master = Tk()
    go = Button(top, text="Change", command=lambda:
                username_options_go(name.get(), top))
    go.grid(row=1, column=1)
    Label(master, text='Quick connect port').grid(row=0)
    Label(master, text='Nickname').grid(row=1)
    quick_connect_port = Entry(master)
    name = Entry(master)
    quick_connect_port.grid(row=0, column=1)
...
def test_message():
    writeToScreen("Hello! This is a LAN chat program by arsho and DOMESCO. ", "System")

def tutor_message():
    writeToScreen(
        "File/Save chat: Save the chat as a txt file. File/Change username: Changing username. File/Exit: Exit to the program. Connect/Quick connect: Connect op ip. Connect/Connect on port: Conect with ip and port. Connect/Disconnect: Disconnect.", "Tutorial"
        )
...
if len(sys.argv) > 1 and sys.argv[1] == "-cli":
    ...
    options_menu.add_command(label="Change username",
                          command=lambda: username_options_window(root))
    ...
    help_menu.add_command(label="Test message", command=lambda:
                        test_message())

    help_menu.add_command(label="Tutorial", command=lambda:
                        tutor_message())    
    ...
    writeToScreen("Your IP address")
    writeToScreen(ip_address)
    main_body_text.config(state=DISABLED)
 
    text_input = Entry(root, width=100)
